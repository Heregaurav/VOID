...
import { ICacheRepository, IDatabaseRepository } from '../../domain/repositories';
import { RoomsUseCase } from '../../application/rooms.usecase';
import { MatchingUseCase } from '../../application/matching.usecase';
import { JWTManager, SessionPayload, AIModerationService } from '../security';
import { ADMIN_TOKEN, MAX_MSG_LENGTH, RATE_LIMIT_WINDOW, RATE_LIMIT_MAX } from '../../config';
import { activeSoulsGauge, activeMatchesGauge } from '../http/server';
...
export function configureSockets(
  ...
  setInterval(async () => {
    try {
      ...
      activeSoulsGauge.set(sessions.length);

      const pairedCount = sessions.filter(s => s.status === 'paired').length;
      activeMatchesGauge.set(pairedCount / 2);
    } catch {
      // Ignore
    }
  }, 10000);
  ...
  io.on('connection', async (socket: ExtendedSocket) => {
    ...
    if (token) {
      payload = JWTManager.verifyToken(token as string);
    }

    // Default if token is missing/expired
    const deviceId = (socket.handshake.query.deviceId as string) || 'void_dev_guest_' + Math.random().toString(36).substring(2, 9);
    ...
    socket.on('admin-ban-user', async ({ socketId, deviceId: banDeviceId, ipAddress: banIp, reason }) => {
      ...
      for (const s of sockets) {
        const extS = s as unknown as ExtendedSocket;
    ...
    socket.on('switch-room', async (newRoom: string) => {
      ...
      if (!session || session.status === 'paired') return;
    ...
    const handleUnpair = async (sId: string, reason: string) => {
      ...
      if (target) {
        if (matchRoom) target.leave(matchRoom);
    ...
    const tryProcessMatching = async () => {
      ...
      for (const pair of pairs) {
        ...
        if (!u1 || !u2) {
          ...
          if (u1) await matchingUseCase.enterQueue(pair.socket1);
          if (u2) await matchingUseCase.enterQueue(pair.socket2);
          continue;
        }
        ...
        if (sock1) sock1.join(pair.roomName);
        if (sock2) sock2.join(pair.roomName);
    ...
    socket.on('join-matchmaking', async () => {
      const session = await cacheRepo.getUserSession(socket.id);
      if (!session || session.status === 'paired' || session.status === 'matching') return;
    ...
    socket.on('skip-match', async () => {
      const session = await cacheRepo.getUserSession(socket.id);
      if (!session || session.status !== 'paired' || !session.partnerId) return;
    ...
    // --- Chat Messages ---
    socket.on('send-message', async ({ text, imageUrl }: { text: string; imageUrl?: string | null }) => {
      const session = await cacheRepo.getUserSession(socket.id);
      if (!session) return;

      const trimmedText = (text || '').trim().slice(0, MAX_MSG_LENGTH);
      if (!trimmedText && !imageUrl) return;

      // Rate limit check
      const now = Date.now();
      if (now - session.windowStart > RATE_LIMIT_WINDOW) {
      ...
      if (trimmedText) {
        const modResult = AIModerationService.moderateText(trimmedText);
    ...
    socket.on('react-message', async ({ messageId, reactionType }) => {
      const session = await cacheRepo.getUserSession(socket.id);
      if (!session) return;
      if (!['skull', 'blood', 'eye'].includes(reactionType)) return;
      ...
      if (session) {
        ...
        if (session.status === 'paired' && session.partnerId) {
          ...
          if (pSession) {
            ...
            if (pSock) pSock.emit('matchmaking-status', 'searching');
            await tryProcessMatching();
          }
        } else if (activeRoom) {
          const leaveMsg = {
